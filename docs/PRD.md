...
| L3 进阶 | 阿杰+ | 多 App | 简易 BYOK、Fallback 链、格式转换、Claude Code 等 | 模型来源页、**模型目录（浏览全部）**、高级设置 |
| L4 极客 | 团队 | 全流量 | 多 BYOK Key、OAuth 精选 Provider（可选）、团队审计 | 套餐团队版、Post-MVP |

**设计原则：** 默认 UI 为 L0–L1；L2+ 功能 **存在但不挡 onboarding**，设置与「更多」中可发现。**模型目录（§5.3.2）** 属 L3，仅经「浏览全部 / 浏览模型」入口进入，不进首屏。

#### 3.6.1 Onboarding 设计原则（TTFV 优先 · 渐进披露）

**核心：不「讲明白产品」，而「最快给到价值」。** 小美激活 = 首次 Chat 成功（§5.13），不是理解中枢架构；强行科普双路径/压缩/记忆会劝退。理解主要发生在**进入 App 后的前 30 秒**，靠「亲手体验一次省钱」而非读说明。

| 手段 | 作用 | 阶段 |
|------|------|------|
| **persona 选择 + ROI 预览**（auth） | 设默认、给净省预期 | 注册时 |
| **首次对话启动 chip**（按 persona 预填示例） | 消除「该问什么」冷启动，<30s 拿到首个好答 | 首次进 Chat |
| **首次回复「啊哈」条**（✓ 完成 + 今日已省 ¥X + 去总览） | 立刻交付「省钱可感知」核心价值 | 首条回复后 |
| **总览「认识 NodeAI」3 步清单**（发消息 / 看省钱 / 连 Cursor，进度 + 可关闭） | 用渐进清单教「中枢」概念，替代弹窗教程 | 首次进总览 |
| **选择焦虑一句话**「不用挑模型，自动选最划算的」 | 打消小白最大焦虑，呼应「全自动」默认（§5.3） | 首屏 / Chat |

**不做：** 上来一段产品导览、术语墙（RTK/BYOK/Gateway）、强制连接 Cursor（IDE persona 才引导）。导览作为 **opt-in**（「带我看看」），默认「直接开始」。
...
**Agent 工作区（MVP · 默认安全目录 + 懒触发，不逼用户上来选文件夹）：**

Chat 也是简单 Agent，会读写文件，必须有落点；但**开场弹文件夹选择器会破坏 TTFV**。原则：

| 设计 | 说明 |
|------|------|
| **默认工作区** | 首次自动创建 `~/Documents/NodeAI`，小美无需选择；Agent 文件默认落此 |
| **懒触发确认** | 仅当 Agent **真要写/删文件**时，在「危险操作确认」卡里顺带告知「将写入 工作区：…（可更改）」，不额外加步骤 |
| **可见可改** | Composer 旁常驻 **工作区 chip**（`📁 工作区 ~/Documents/NodeAI ▾`）→ 弹层显示路径 + 「更改文件夹…」；设置 →「Agent 工作区」亦可改 |
| **纯文本任务不落盘** | 写文案 / 改一段文字等产出留在对话，不创建文件；只有「保存到 / 改某文件」才用到工作区 |
| **进阶（Post-MVP）** | 每个对话可单独指定工作区；连 IDE 时默认跟随当前打开项目 |

**安全边界：** 写 / 删操作弹窗确认（设置「危险操作需确认」默认 ON）；**禁止**静默访问系统目录、Keychain、`.env` 等敏感路径（规则列表）。
...
Vercel AI Gateway 含 **100+ 模型**。原则：**首屏不暴露目录**——小美停在「全自动」永远不必看到上百个模型；目录是 **L3 进阶面**，服务知道自己要什么的用户，同时**用真实价格 + 厂商建立信任**（「背后真有 100+ 模型」）。

**入口：** 首屏「固定某模型 · 浏览全部」按钮 + 「更多」菜单「浏览模型」；以**模态**打开，不占首屏。

**分类是「搜索优先 + 多维过滤」，不是单一维度：**

| 维度 | 角色 | 说明 |
|------|------|------|
| **搜索** | 第一控件（最突出） | 模糊匹配 **友好名 + slug + 厂商**（搜 `sonnet`/`flash`/`qwen` 可中）；100+ 时是最快路径 |
| **能力类型**（主轴） | 顶部分段 | 对话 / 生图 / 视频 / 嵌入（对齐九类 API）；先决定「要哪类」 |
| **厂商**（次轴） | 过滤 chips（带 logo） | 全部 / Anthropic / OpenAI / Google / xAI / DeepSeek …；服务品牌忠诚用户，logo 强化信任 |
| **排序 / 附加过滤** | 下拉 + badge | 推荐 / 最便宜 / 最快 / 上下文最长；能力标签（工具调用、视觉、推理、长上下文） |

**降噪策略（避免一上来 100 行）：**

- 默认只露 **精选 ~6–8**，下方「全部模型 · N」展开完整可搜目录。
- **「我的常用 / 最近用」置顶**（个人收藏，本地持久化）——100+ 时价值极高。
- 目录顶部保留 **「让全自动决定（推荐）」** 回退入口，提醒用户「其实不用挑」。

**每行信息：** 厂商 logo + 友好名 + `slug`（mono，供阿杰复制到 IDE）+ 入/出价格 + 上下文 badge + 能力 chips + ★收藏；当前固定项高亮。

**与凭证抽象一致（§5.15）：** 目录仅用于**浏览与固定**，不暴露 Provider OAuth / Key 配置入口。
...
### 7.1 首次使用（Happy Path ≤ 2 分钟 · TTFV 优先）

1. 安装并打开 NodeAI → 登录（或零登录本地模式）
2. Onboarding：选 persona → 看预计净省（ROI）→ 进入
3. 进入对话：默认 **全自动**（不必挑模型）；首屏给 **persona 启动 chip**（一键预填示例）
4. 发出第一条消息 → 回复后弹 **「✓ 第一次对话完成 · 今日已省 ¥X」**「啊哈」条，引导去总览
5. 首次进总览：**「认识 NodeAI」3 步清单**（发消息 ✓ / 看省钱 / 连 Cursor）渐进引导
6. 若用 Cursor：去 **连接应用** → 复制共用地址 + 专用接入码，Model 填 `nodeai-auto`（§5.3.1）

**激活定义：** 小美 = 第 4 步（首次 Chat 成功），**非**必须连 Cursor；IDE/团队 persona 才在第 6 步引导连接。

**工作区：** 不在 onboarding 逼用户选文件夹；默认 `~/Documents/NodeAI`，仅在 Agent 首次写文件时于确认卡告知、Composer chip 常驻可改（§5.1）。
...
| 原型（`prototypes/`） | PRD |
|---------------------|-----|
| 对话 + 场景 chip + 启动 chip / 啊哈条 | §5.1、§5.13、§3.6.1 |
| 选模型（全自动 + 场景 + 浏览全部目录） | §5.3、§5.3.1、§5.3.2、§5.4 |
| 总览 · 九类 API 网格 + 「认识 NodeAI」清单 | §5.10、§3.6.1 |
| 连接应用（Model = nodeai-auto） | §5.5、§5.6、§5.3.1 |
| 记忆 | §5.8 |
| 模型来源 + Fallback | §5.12、§5.4 |
| 账单 / 套餐 | §5.14、§3.5 |
| 设置（本地服务暂停 / 端口） | §5.7、§5.11、§5.2 |
| auth Onboarding | §3.6、§3.6.1、§4 |
