...
| 选项 | 行为 |
|------|------|
| **智能路由**（默认） | 按任务自动选模型（§5.4） |
| **具体模型** | 如 Claude Sonnet 4、Gemini Flash — 本对话 / 全局默认固定 |

- 下拉第一项永远是「智能路由」，下列精选模型（友好名 + 一行说明）
- **不要**分段控件（智能 | 档位 | 自选）、不要单独路由页
- 外部 App 仍可用 `nodeai-auto`（跟随对话里的选择）或具体 model slug

**请求优先级：** 请求体 `model` 字段 > App 级默认 > 对话顶栏选择 > Fallback
...
| Models List | 聚合 Gateway + 用户 BYOK + NodeAI 虚拟模型别名 |
| 认证 | Bearer Token = App Key（见 5.6） |
| 协议转换 | 请求入站 OpenAI 形态 → 按上游 **格式** 转 Anthropic / Gemini（§5.11） |

**虚拟模型别名：** 见 §5.3；Models List 返回 Smart + 精选目录。
